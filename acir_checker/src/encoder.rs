...
impl<'a, F: AcirField> Translator<'a, F> {
    ...
    pub(crate) fn translate_witness_map(&mut self, witness_map: WitnessMap<F>) -> Result<(), Error> {
        for (witness, value) in witness_map {
            // Skip witnesses not declared in the SMT formula. When a circuit has only constant
            // assertions (num_vars = 0), next_witness_index = 0 and no SMT variables exist —
            // referencing them would cause a solver parse error.
            if witness.witness_index() >= self.next_witness_index {
                continue;
            }
            if self.use_int {
    ...
    pub(crate) fn translate_to_smt(
        ...
        let mut mem_traces: HashMap<BlockId, MemTrace> = HashMap::new();
    ...
    fn translate_memory_op(
        ...
        let index_wit = self.new_const(index);
        // Ensure index is in range
        self.translate_range_inclusive(index, len as u32)?;

        let value_wit = self.new_const(value);

        if op == MemOpKind::Read {
            // Read operation
            // index != op.value == x0
            for i in 0..len {
    ...
    fn translate_memory_op_int(
        ...
        let index_wit = self.new_const_int(index);
        // Ensure index is in range
        self.translate_range_inclusive_int(index, len as u32)?;

        let value_wit = self.new_const_int(value);
    ...
    fn translate_assert_zero(&mut self, expression: &Expression<F>) -> Result<(), Error> {
        if self.use_int {
    ...
    fn translate_range(&mut self, input: &FunctionInput<F> , mut num_bits: u32) -> Result<(), Error> {
    ...
    fn translate_range_int(&mut self, witness: Witness, num_bits: u32) -> Result<(), Error> {
        if num_bits == 0 {
            let wit = self.new_const_int(witness);
            let zero = self.zero_int();
    ...
    fn translate_range_ff(&mut self, witness: Witness, num_bits: u32) -> Result<(), Error> {
        if num_bits == 0 {
            let wit = self.new_const(witness);
            let zero = self.zero();
            self.solver.assert(wit.eq(zero))?;
    ...
    fn translate_range_inclusive(&mut self, witness: Witness, length: u32) -> Result<(), Error> {
        assert!(length > 0);
        let bound = length - 1;
        if bound == 0 {
            let wit = self.new_const(witness);
    ...
    fn translate_range_inclusive_int(&mut self, witness: Witness, length: u32) -> Result<(), Error> {
        assert!(length > 0);
        let bound = length - 1;
        if bound == 0 {
            let wit = self.new_const_int(witness);
    ...
    fn encode_and_ff(&mut self, lhs: Witness, rhs: Witness, num_bits: u32, output: Witness) -> Result<(), Error> {
        if num_bits == 0 {
    ...
    fn encode_and_int(&mut self, lhs: Witness, rhs: Witness, num_bits: u32, output: Witness) -> Result<(), Error> {
        if num_bits == 0 {
    ...
    fn encode_bitsum_int(&mut self, witness: Witness, num_bits: usize) -> Result<Vec<Int>, Error> {
        ...
        for (i, b) in bits.iter().enumerate() {
            let b_int = b;
            let coeff = BigUint::from(2u32).pow(i as u32);
            terms.push(Int::rmul(vec![Int::new_value(&coeff.to_string()), b_int.clone()]));
        }
    ...
    fn new_witness_int_bool(&mut self) -> Result<Int, Error> {
    ...
    fn encode_bitsum(&mut self, witness: Witness, num_bits: usize) -> Result<Vec<FField>, Error> {
        // assert!(num_bits > 0);
    ...
    fn encode_bool(&mut self) -> Result<FField, Error> {
    ...
    // Create a field constant out such that out in \{0,1}.
    fn encode_base(&mut self, base: u32) -> Result<FField, Error> {
        let res = self.new_witness()?;
        let zero = FField::zero();
    ...
    fn encode_basesum(&mut self, witness: Witness, num_bits: u32, base: u32) -> Result<Vec<FField>, Error> {
    ...
    fn encode_range_explicit_values(&mut self, witness: Witness, num_bits: u32) -> Result<(), Error> {
    ...
    fn encode_int_range(&mut self, witness: Witness) -> Result<(), Error> {
        let wit = self.new_const_int(witness);
        let minus_one = Self::minus_one_int();
        self.solver.assert(minus_one.lt(wit.clone()))?;
    ...
    fn new_witness(&mut self) -> Result<FField, Error> {
        let new_wit = Witness(self.next_witness_index);
        let new_wit_name = witness_name(new_wit);
        self.next_witness_index += 1;
        self.solver.declare_const(&new_wit_name, Type::FField)?;
        Ok(FField::new_const(&new_wit_name))
    }

    fn new_witness_int(&mut self) -> Result<Int, Error> {
        let new_wit = Witness(self.next_witness_index);
        let new_wit_name = witness_name(new_wit);
        self.next_witness_index += 1;
        self.solver.declare_const(&new_wit_name, Type::Int)?;
        self.encode_int_range(new_wit)?;
        Ok(Int::new_const(&new_wit_name))
    }

    fn new_cond_lit(&mut self, opcode_loc: usize) -> Result<Bool, Error> {
    ...
    pub(crate) fn has_constraints(&self) -> bool {
        self.solver.has_constraints()
    }
}

pub(crate) fn num_vars<F: AcirField>(circuit: &Circuit<F>) -> u32 {
    let witness_count = if let Some(max_index) = max_witness_index(circuit) {
        max_index + 1
    } else {
        0
    };
...
pub(crate) fn max_witness_index<F: AcirField>(circuit: &Circuit<F>) -> Option<u32> {
    let mut max: Option<u32> = None;

    let mut bump = |w: Witness| {
        let idx = w.witness_index();
        max = Some(max.map_or(idx, |m| m.max(idx)));
    };

    let bump_expr = |expr: &Expression<F>, bump: &mut dyn FnMut(Witness)| {
    ...
    for opcode in &circuit.opcodes {
        match opcode {
            acir::circuit::Opcode::AssertZero(expr) => {
                bump_expr(expr, &mut bump);
            }
            acir::circuit::Opcode::BlackBoxFuncCall(bb) => {
                for w in bb.get_input_witnesses() {
                    bump(w);
                }
            ...
            acir::circuit::Opcode::BrilligCall { inputs, outputs, predicate, .. } => {
                for input in inputs {
                    match input {
                        BrilligInputs::Single(expr) => bump_expr(expr, &mut bump),
                ...
                for output in outputs {
                    match output {
                        BrilligOutputs::Simple(w) => bump(*w),
                ...
                bump_expr(predicate, &mut bump);
            }
            acir::circuit::Opcode::Call { inputs, outputs, predicate, .. } => {
                for &w in inputs {
                    bump(w);
                }
                for &w in outputs {
