...
use nargo_toml::{
    ManifestError, PackageSelection,
    get_package_manifest, resolve_workspace_from_toml,
};
...
const NAVE_VERSION: &str = env!("CARGO_PKG_VERSION");
...
#[derive(Parser, Debug)]
#[command(name="nave", author, version=VERSION_STRING, about, long_about = None)]
struct NaveCli {
    #[command(subcommand)]
    command: NaveCommand,
...
pub(crate) fn start_cli() -> eyre::Result<()> {
    ...
    match command {
        NaveCommand::FormalVerify(args) => with_workspace(args, config, fv_cmd::run),
    }?;
...
fn with_workspace<C, R>(cmd: C, config: NaveConfig, run: R) -> Result<(), CliError>
where
    C: WorkspaceCommand,
